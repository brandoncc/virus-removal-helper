class ProgressBar
  ...
  def build_bar(c, width, completed_char = '#', pad_char = '-')
    ...
    inner_bar_size = width - 4 - completion_percentage

    inner_bar = ''
    inner_bar << completed_char * (inner_bar_size * completion_percentage).ceil
    inner_bar << pad_char * (inner_bar_size - inner_bar.length)

    "[#{inner_bar}] #{(completion_percentage * 100).round}%"
  end

  def humanize_size(size)
    if @total_size >= 1024 * 1024 * 1024 * 1024
      "#{(size / (1024.0 * 1024 * 1024 * 1024)).round(2)}TB / #{(@total_size / (1024.0 * 1024 * 1024 * 1024)).round(2)}TB"
    elsif @total_size >= 1024 * 1024 * 1024
      "#{(size / (1024.0 * 1024 * 1024)).round(2)}GB / #{(@total_size / (1024.0 * 1024 * 1024)).round(2)}GB"
    elsif @total_size >= 1024 * 1024
      "#{(size / (1024.0 * 1024)).round(2)}MB / #{(@total_size / (1024.0 * 1024)).round(2)}MB"
    elsif @total_size >= 1024
      "#{(size / (1024.0)).round(2)}KB / #{(@total_size / (1024.0)).round(2)}KB"
    else
      "#{size}B / #{@total_size}B"
    end
  end
